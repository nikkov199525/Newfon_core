...
/* Transition from a hard sonorant to the following vowel.
   Indexed by the vowel phoncode. Newfon always closes a sonorant with
   such a transition; ru_tts only does it before a hard consonant. */
                  ...
                  if ((a != PH_J) || (transcription[i + 1] > PH_MINUS) || ((c > PH_I) && (c < PH_COMMA   )))
                    {
                      ...
                      else put_sound(script, a + ((a < PH_J) ? soundset5[c] : 99), 3);
                    }
                          ...
                          if (j > PH_I)
                            {
                              if (j < PH_TERMINATOR)
                                ...
                                /* A wider break, that is a hyphen between
                                   words. Newfon closes the vowel with its own
                                   transition here. */
                                put_sound(script, a + ((a != PH_I) ? 117 : 116), 4);
