...
static int check_clause_termination(input_t *input, sink_t *consumer)
{
  ...
  if (s)
    {
      char c = *(input->start++);
      /* Newfon closes the text as if a comma followed it. The comma is not
         part of the text, so numbers and single characters are unaffected;
         it only selects the clause type and the interclause gap. */
      ...
      if (i >= (sizeof(clause_terminations) / sizeof(uint16_t)))
        {
          /* Unknown combination. Newfon falls back to the clause type of
             this punctuation followed by a space instead of taking the
             very first table entry. */
          termination = PAIR(c, ' ');
          for (i = 0; (i < (sizeof(clause_terminations) / sizeof(uint16_t))) && (clause_terminations[i] != termination); i++);
        }
      ...
      for (transcription->flags = CLAUSE_START; (input.start < input.end) && (consumer->buffer_offset < TRANSCRIPTION_MAXLEN) && !consumer->status; input.start++)
        {
                  ...
                  if (((c != '+') && (c != '=')) || (prev > PH_I))
                    {
                      ...
                      put_transcription_block(consumer, char_index);
                      /* Newfon closes a hyphen with a blank as well, so that
                         a hyphenated word is spoken as two words. */
          ...
          if (IS_DIGIT(c))
            {
              /* Newfon keeps the number in the same clause with the text
                 that precedes it, so nothing is flushed here. */
