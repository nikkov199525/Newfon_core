...
/* Parameters of the adaptive crossfade algorithm of ru_tts */
...
static void sink_put_clamp(sink_t *consumer, float sample)
{
              ...
              while (l >= ax)
                {
                  ...
                  if (use_new_algo)
                    ...
                    {
                      /* The crossfade is evaluated with the extra bit of
                         precision the original engine uses: the samples
                         are scaled by 128 before the division and the
                         result is taken from the high byte of the
                         doubled value. */
                      int16_t current = ((int16_t)(script->voice->samples[sidx++])) << 7;
                      int16_t value = (int16_t)((((int32_t)((ax << 7) - current)) * dx++) / l);
                      value = (int16_t)((value + current) << 1);
                      sink_put(consumer, (int8_t)(value >> 8));
                      ax = ((++j) < next_pattern_offset) ? ((int16_t)(script->voice->samples[j])) : 0;
                    }
                  while ((--k) && (--scnt));
                  if (k)
