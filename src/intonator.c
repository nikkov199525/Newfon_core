 ...
 *
 * The tone range comes from the voice definition, the pitch and the
 * inflection only shift and widen it. The arithmetic below is the one
 * the original Newfon add-on performed before feeding set_pitch_min()
 * and set_pitch_max(), truncation towards zero included.
 */
void modulation_setup(modulation_t *modulation, int voice, int pitch, int inflection)
{
  const newfon_voice_t *v = &newfon_voices[((voice < 0) || (voice >= NEWFON_VOICE_COUNT)) ? 0 : voice];
  int mintone = v->mintone;
  int maxtone = v->maxtone;
  int shift = (int) ((((double) (maxtone - mintone)) / 50.0) * (((double) (inflection - 50)) / 2.0));

  mintone -= shift;
  maxtone += shift;
  shift = (int) (((double) (pitch - 50)) / 1.3);
  mintone += shift;
  maxtone += shift;

  if (mintone < NEWFON_TONE_MIN)
    mintone = NEWFON_TONE_MIN;
  else if (mintone > NEWFON_TONE_MAX)
    mintone = NEWFON_TONE_MAX;
  if (maxtone < NEWFON_TONE_MIN)
    maxtone = NEWFON_TONE_MIN;
  else if (maxtone > NEWFON_TONE_MAX)
    maxtone = NEWFON_TONE_MAX;
  if (maxtone < mintone)
    maxtone = mintone;

  modulation->mintone = (uint16_t) mintone;
  modulation->maxtone = (uint16_t) maxtone;
}
                      ...
                      modulation_t *modulation, uint8_t clause_type)
{
  ...
  if (nspeechmarks)
      ...
      for (i = 0; i < soundscript->length; i++)
        {
          ...
          if (soundscript->voice->sound_lengths[j] < soundscript->threshold)
            {
      ...
      for (i = 0; i < soundscript->length; i++)
        {
          uint8_t j = soundscript->sounds[i].id;
          uint8_t k = soundscript->sounds[i].stage;
          if ((prevk != ((uint16_t)k)) && (soundscript->voice->sound_lengths[j] < soundscript->threshold))
            {
              int q = 0;
  ...
  else
    {
      for (i = 0; i < NSTAGES; i++)
        {
          soundscript->icb[i].stretch = soundscript->threshold;
          soundscript->icb[i].delta = 0;
        }
